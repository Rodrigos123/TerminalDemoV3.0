...
def _read_account(monitor_dir: Path) -> Dict[str, Any]:
    ...
    return {
        "balance_usdt": _num(obj.get("balance_usdt"), 0.0),
        "equity_total": _num(obj.get("equity_total"), 0.0),
        "upl_total": _num(obj.get("upl_total"), 0.0),
...
def _split_tsv_line(line: str) -> List[str]:
    """
    Si toda la línea está entre comillas, quita las comillas externas y luego split por TAB.
    """
    s = line.rstrip("\r\n")
...
def _read_closed_from_csv(root: Path) -> List[Dict[str, Any]]:
    """
    Lee trade_log.csv donde cada línea puede venir completa entrecomillada.
    - Normaliza encabezado (BOM/espacios) y valida campos requeridos.
    - Devuelve solo filas con Close time no vacío.
    """
...
def render(root: Path, closed_n=10, err_n=5) -> List[str]:
    ...
    out += [
        ...
        f"Saldo USDT: {_fmt_thousands0(acct['balance_usdt'])}   {V}   Patrimonio: {_fmt_thousands0(acct['equity_total'])}   {V}   Open P/L: {acct['upl_total']:+.2f}",
        H_LIGHT * 100,
    ]
    ...
    for s in statuses:
        try:
            if (s.get("status") == "OPEN") and _num(s.get("lots"), 0.0):
                sym = s.get("symbol", "")
                lots_by_sym[sym] = lots_by_sym.get(sym, 0.0) + float(_num(s.get("lots"), 0.0) or 0.0)
        except Exception:
            continue

    out += ["Lotes Abiertos", H_LIGHT * 30, f"{'SYMBOL':<14}{SEP}{'TOTAL LOTS':>12}", H_LIGHT * 30]
    if lots_by_sym:
        for sym in sorted(lots_by_sym.keys()):
            out.append(f"{sym:<14}{SEP}{_fmt_lots6(lots_by_sym[sym]):>12}")
    else:
        out.append("(sin posiciones abiertas)")
    out.append(H_LIGHT * 30)
    ...
    for st in sorted(statuses, key=lambda x: (int(x.get("magic") or 0))):
        try:
            ...
            open_lots = _fmt_lots6(st.get("lots"))
            open_pl   = _fmt_pl6(st.get("open_pl"))
            open_time = st.get("open_time", "") or ""
            dur = f"{_dur_hours(open_time, now_iso):.2f}h" if status == "OPEN" else ""
    ...
    # Cerradas desde CSV (TSV con filas entrecomilladas)
    ...
    closed_cols = [
        ("CLOSE TIME", 19, 'l'), ("TICKET", 19, 'l'), ("SYMBOL", 10, 'l'), ("S", 1, 'l'),
        ("LOTS", 11, 'r'), ("OPEN@PRICE", 11, 'r'), ("CLOSE@PRICE", 12, 'r'),
        ("NET", 10, 'r'), ("DUR", 7, 'r'), ("EXIT", 11, 'l')
    ]
    closed_block = table_block("OPERACIONES CERRADAS", closed_cols, closed_rows)

    if closed_rows:
        widths = [w for _, w, _ in closed_cols]
        sep_len = len(SEP); net_idx = 7
